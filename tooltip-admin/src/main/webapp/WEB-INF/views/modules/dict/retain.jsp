...
jQuery(function($) {
    ...
    checkArrCookie = $.cookie('dictList');

	if(checkArrCookie!=null&&checkArrCookie.length>0) {
        checkArrCookie = checkArrCookie.split(",");
        for(var i=0;i<checkArrCookie.length;i++){
            //console.log(checkArr[i]);
            $("#"+checkArrCookie[i]).attr("checked",true);
        }
        selectAction();
    }else{
        selectAction();
	...
	function selectAction() {
        ...
        $.cookie('dictList', checkArrCookie, options);
	}
});
