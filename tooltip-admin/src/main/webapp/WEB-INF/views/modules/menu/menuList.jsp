...
<html>
	<head>
		...
		<meta name="menu" content="${ctx}/menu/list" />
	</head>
	...
	<body>
		...
		<div class="tabbable">
			<ul class="nav nav-tabs">
				<li class="active">
					<a href="${ctx}/menu/list">
						菜单列表
					</a>
				</li>
		
				<li>
					<a href="${ctx}/menu/form">
						菜单添加
					</a>
				</li>
		...
		<div class="widget-box" style="margin:0px 0px 6px 0px;">
			<div class="widget-body">
				<div class="widget-main">
					<form class="form-search" id="searchForm" action="${ctx}/menu" method="post">
				...
				<c:forEach items="${list}" var="menu">
					<tr id="${menu.id}" data-tt-id="${menu.id }" data-tt-parent-id = "${menu.parent.id}" >
						<td>
							...
							</i><a href="${ctx}/menu/form?id=${menu.id}">${menu.name}</a>
						</td>
						<td>${menu.href}</td>
						...
						<td>
							<a href="${ctx}/menu/form?id=${menu.id}">修改</a>
							<a href="${ctx}/menu/delete?id=${menu.id}" onclick="return confirmx('要删除该菜单及所有子菜单项吗？', this.href)">删除</a>
							<a href="${ctx}/menu/form?parent.id=${menu.id}">添加下级菜单</a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
